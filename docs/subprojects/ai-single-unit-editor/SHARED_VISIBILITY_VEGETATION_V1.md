...
This document describes the contract implemented on the temporary integration branch based on draft PR #127. It is not part of `real-wargame-preview` until the integration PR is accepted and transferred.
...
- navigation carried separate movement values;
...
The existing serialized `forest: 0 | 1 | 2` layer remains valid. The explicit forest layer wins. For legacy maps, `terrain='forest'` with `forest=0` resolves as sparse forest inside core consumers.
...
PR #127 remains authoritative for simulation phase order and per-unit AI scheduling:
