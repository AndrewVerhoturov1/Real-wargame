...
Показать пользователю временную ветку feat/view-memory-heatmap-temp для ручной проверки. Переносить её в real-wargame-preview только по отдельной явной команде; main не менять без отдельного явного GO пользователя.
