...
|---|---|---|---|---|---|
| [AI Single-Unit Editor — Stateful Tactical Awareness, Hierarchical States and Plans](ai-single-unit-editor/STATUS.md) | `ai-single-unit-editor` | active | Tactical Query System cover vertical slice is implemented on an isolated temporary branch: Graph v2 explicitly generates, filters, scores and selects cover positions. | Run the prepared visual QA only after explicit user approval. Do not transfer the temporary branch to real-wargame-preview without a separate command. | 2026-07-14 |
