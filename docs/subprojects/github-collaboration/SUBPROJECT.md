...
- Временные task-ветки (`codex/*`) закрываются после переноса результата в целевую preview-ветку (`real-wargame-preview`). Исключения фиксируются в отчёте с явной причиной.
