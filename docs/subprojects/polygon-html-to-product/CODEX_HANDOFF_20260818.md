...
## Дополнение: типографика «Профилей маршрута»

Последний локальный product commit для этого дополнения: `c5c060f75f3753a229516ffff1e6f230d33252cd`.

В `polygon-global-editor-feature-grid.css` у `.polygon-route-profile-editor` явно закреплён исходный стек HTML `Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Arial, sans-serif`; без него портал редакторов фактически наследовал `Arial, Helvetica, sans-serif`. Все кнопки, вводы, списки и textarea этого дерева наследуют тот же стек. Единственное намеренное исключение — `ge-inline-number input`, который использует исходный mono-стек прототипа. Не убирать эти правила при дальнейших визуальных правках.

Browser QA на локальном приложении при `1442×740` прошёл для `Основное`, `Местность`, `Тактика` и `Маршрут`; ошибок консоли/страницы нет. Изменение только presentation; реальные owners и функции не изменялись. Этот commit локальный: он не отправлен, не перенесён в preview и не задеплоен.
...
### Дополнение: внутренние вкладки профиля маршрута

Локальный product commit после основного прохода:

```text
e26116d6f7cf19f02e895319a38f8fb372e5222c
```

Он не опубликован, не перенесён в preview и не задеплоен. На existing owner fields перенесена композиция HTML для трёх внутренних вкладок `Профили маршрута`:

- `Местность` — девять настоящих terrain/slope полей в принятой сетке карточек;
- `Тактика` — четыре настоящих веса, честные метки неподключённых факторов и сворачивание будущих территориальных весов;
- `Маршрут` — owner-driven maximum-route-cost, правила и параметры перепланирования в принятом порядке.

Встроенный браузер при `1442×740` подтвердил визуальные состояния всех трёх вкладок. Для `maximumRouteCost` подтверждён owner readback `пусто → 1 → пусто`; блок территории не виден на вкладке `Маршрут`. Не заменять эти связи отдельным local UI state и не выдавать счётчики usage-footer за LIVE capability.
