...
**ИСТОРИЧЕСКИЙ ДОКУМЕНТ.** Эта six-X волна уже выполнена и сохранена checkpoint'ом в `real-wargame-preview`. Содержимое ниже — историческая запись плана, границ и результатов той волны, а не текущий план работ. Текущий процесс подпроекта — поэлементный перенос:

`docs/subprojects/polygon-html-to-product/ELEMENT_MIGRATION_WORKFLOW.md`

Результаты этой волны используются как исторические исследования и технический контекст и не считаются автоматическим основанием для объединения кода.
