...
- проект использует PixiJS 8; архитектура и статус кампании зафиксированы в `docs/architecture/PIXIJS8_MIGRATION_CAMPAIGN.md`;
- успешно интегрированная миграция PixiJS 8 не равна подтверждённому запуску на ПК пользователя: текущий Windows launcher blocker должен отражаться честно;
- нельзя заявлять проверки, которые фактически не выполнялись;
- visual QA запускается только после явного разрешения пользователя и не считается успешным без свежих просмотренных PNG.
