...
## Простая проверка пунктов 4–5

Если пункты 1–3 уже хорошие, дальше можно не читать JSON руками.
...
Ожидание в нижнем блоке `Validation / Engine result`:
...
```text
пункт 4 — редактор видит local engine;
пункт 5 — проверка графа и тестовое решение идут через local engine, а не через браузерную вкладку.
```

## Подробная проверка кликами
