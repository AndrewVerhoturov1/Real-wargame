...
Q-задача не требует ZIP. Результат Q — отдельная ветка, Pull Request и короткий отчёт для Codex. Для W-class X-режима (сильное reasoning) используется тот же механизм доставки через GitHub с markdown-отчётом.
