...
5. Временная task-ветка закрывается/удаляется. Временный PR, если он больше не нужен, также закрывается. Preview-ветка r-init удаляется.
6. Если ветка не закрыта — в отчёте указывается причина: работа не завершена, есть незакрытый конфликт, пользователь просил оставить ветку, PR нужен для обсуждения, или перенос в preview не подтверждён. Отчёт обязан содержать фразу «Временная ветка оставлена открытой, потому что ...».
7. Codex обновляет статусы компонентов.
