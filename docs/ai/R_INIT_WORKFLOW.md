...
1. Изменения готовятся во временной task-ветке.
2. Codex переносит результат в целевую preview-ветку (`real-wargame-preview`) и создаёт или обновляет preview-PR в `main`.
3. До закрытия временной ветки Codex проверяет, что изменения действительно присутствуют в `real-wargame-preview` и что нерешённых конфликтов нет.
4. После подтверждённого переноса временная task-ветка закрывается/удаляется до начала ручного тестирования. Временный PR, если он больше не нужен, также закрывается. Preview-ветка и preview-PR остаются открытыми для тестирования.
5. В отчёте указываются временная ветка, способ переноса результата в preview, статус закрытия/удаления ветки и временного PR. Ветка может остаться открытой только если работа не завершена, есть нерешённый конфликт, пользователь просил оставить ветку, PR нужен как место обсуждения или перенос в preview не подтверждён. Тогда отчёт обязан содержать фразу «Временная ветка оставлена открытой, потому что ...».
6. Preview-PR помечается меткой `[r-init]` или `[preview]` в заголовке или описании.
7. К preview-PR прикладывается `.bat`-лаунчер (см. раздел «Launcher»).
...
3. Codex выполняет merge preview-PR в `main`.
4. Codex проверяет наличие изменений в `main` и отсутствие нерешённых конфликтов.
5. Preview-ветка r-init удаляется, если она больше не нужна.
6. Codex обновляет статусы компонентов.
...
Временная task-ветка: <имя ветки>
Как результат попал в preview: <merge / cherry-pick / другой способ>
Временная task-ветка закрыта/удалена: да / нет
Временный PR закрыт: да / нет / не применимо
Если ветка оставлена открытой: Временная ветка оставлена открытой, потому что <причина>
