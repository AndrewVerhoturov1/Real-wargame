...
1. Codex выкладывает изменения в целевую preview-ветку (`real-wargame-preview`) и открывает preview-PR.
...
6. Временная task-ветка (из которой изменения попали в preview) закрывается/удаляется. Если ветка не закрыта — в отчёте указывается причина.
