...
1. Результат из временной task-ветки переносится в целевую preview-ветку (`real-wargame-preview`) и открывается или обновляется preview-PR.
2. Codex проверяет, что изменения действительно попали в preview и что нерешённых конфликтов нет.
3. Временная task-ветка закрывается/удаляется до ручного тестирования; временный PR, если он больше не нужен, также закрывается.
4. Если применяется исключение, отчёт указывает ветку, способ переноса, статус закрытия и фразу «Временная ветка оставлена открытой, потому что ...».
5. К preview-PR прикладывается `.bat`-лаунчер для запуска одним кликом.
6. Codex даёт человеку чеклист ручного тестирования.
7. Человек тестирует и говорит GO или NO-GO.
8. После GO Codex выполняет merge preview-PR и фиксирует статусы компонентов.
