...
Codex выполняет merge, закрывает временную task-ветку (и preview-ветку r-init, если она не нужна), обновляет статусы компонентов. Если ветка не закрыта — в отчёте указывается причина, включая фразу «Временная ветка оставлена открытой, потому что ...».
