...
1. Переносит результат из временной task-ветки в целевую preview-ветку (`real-wargame-preview`) и открывает или обновляет preview-PR.
2. Проверяет, что изменения действительно присутствуют в preview и что нерешённых конфликтов нет.
3. До ручного тестирования закрывает/удаляет временную task-ветку и закрывает временный PR, если он больше не нужен.
4. В отчёте указывает временную ветку, способ переноса результата в preview, статус закрытия ветки и PR. Исключения допустимы только если работа не завершена, есть конфликт, пользователь просил оставить ветку, PR нужен для обсуждения или перенос не подтверждён; тогда обязательна фраза «Временная ветка оставлена открытой, потому что ...».
5. Добавляет `.bat`-лаунчер для терминал-фри запуска.
6. Даёт человеку чеклист ручного тестирования.
7. Ждёт GO или NO-GO.
...
Codex выполняет merge preview-PR, удаляет preview-ветку r-init, если она больше не нужна, и обновляет статусы компонентов.
...
Codex фиксирует handoff-шаблон: GO-статус, чеклист, использованную временную task-ветку, способ переноса результата в preview, статус закрытия временной ветки и PR, а также статусы компонентов (included in preview / excluded / blocked / superseded / needs follow-up).
