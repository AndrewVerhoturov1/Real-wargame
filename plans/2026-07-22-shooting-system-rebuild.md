...
После этапа 3 используются стабильные корни:
...
`UnitInfantryCombatState` хранит equipment, active actions, aim, recoil, wounds, blood loss, fatigue, suppression и capabilities.
...
| 3 | Один полный винтовочный выстрел | `feature/20260722-shooting-stage-03-rifle-shot` | 1B, 2B |
| 4 | Пакетный ProjectileRuntime и benchmark 100×100 | `feature/20260722-shooting-stage-04-projectile-benchmark` | 3 |
...
- `package.json` — три focused scripts.
...
- invalid IDs/numbers/references;
- immutable published revision;
- publish creates next revision;
- archived exact resolution;
- built-in bundle valid;
- import/export byte equality;
- порядок arrays/maps не влияет на output;
...
- `src/core/infantry-combat/actions/PostureCoordinatorAdapter.ts`
- `src/core/infantry-combat/actions/MovementCoordinatorAdapter.ts`
- `src/core/infantry-combat/actions/WeaponPreparationCoordinatorAdapter.ts`
...
- stale ownerToken не отменяет новое действие;
- channel locks сохраняются;
- cancel/complete exactly once;
- save/load running/waiting actions;
- Graph v2 решений нет.

### Откат

Удалить adapters и вернуть unit/scene fields к этапу 1B; старый posture runtime остаётся.

## 11. Этап 3 — один полный винтовочный выстрел
...
- фазы accepted → weapon_ready → aiming → firing → recovery;
...
- один impact;
- save/load во всех критических точках.

Stage 3 поддерживает только `single`; массовая оптимизация — stage 4. Новый код не импортирует старую стрельбу.

### Откат

Удалить `infantryCombat` state и новые modules/tests. Legacy path остаётся включённым до stage 15.

## 12. Этап 4 — ProjectileRuntime и benchmark
...
Условно создать `src/core/spatial/UnitSpatialIndex.ts` только если измерение доказывает путь `activeProjectiles × state.units.length` и в актуальном preview отсутствует пригодный канонический unit query. Созданный индекс становится единственным общим динамическим unit index.

### Benchmark

Фиксирует active projectiles, fixed steps, broad/narrow tests, impacts, near-miss candidates, saturation, allocations after warmup и simulation cost average/p50/p95/p99/max. После измерения hard caps записываются в `ProjectileRuntimeConfig`.
...
Вернуть минимальный stage-3 runtime.

## 13. Этап 5 — aim, lead и recoil
...
Изменить `FireTaskRuntime.ts`, `ShotCommitService.ts`, `InfantryCombatState.ts`, `SceneExport.ts`, `package.json`.

Tracking читает только `PerceptionContactMemory` и обновляется 5 Hz. Probability — только факт. Seed dispersion зависит от stable IDs. Diagnostics раскладывает base/aim/movement/posture/fatigue/wounds/skill/proficiency/recoil.

Откат: stage-3 прямое направление используется только как migration fallback старых snapshots.

## 14. Этап 6 — hit, penetration и wounds
...
- шесть smoke wrapper/source files для geometry, wounds и penetration.

Изменить projectile resolver/runtime, `InfantryCombatState.ts`, `SceneExport.ts`, `package.json`.
...
Откат: impacts снова terminal без wound application.

## 15. Этап 7 — blood loss, fatigue и first aid
...
- шесть smoke wrapper/source files для blood, fatigue и first aid.

Изменить `WoundRuntime.ts`, `PhysicalCapabilities.ts`, `InfantryCombatState.ts`, `SimulationTick.ts`, `SceneExport.ts`, `package.json`.
...
Откат: stage-6 wounds сохраняются без progression.

## 16. Этап 8 — автоматический огонь и suppression
...
- шесть smoke wrapper/source files для cadence, physical sources и performance.

Изменить `FireTaskRuntime.ts`, `ShotCommitService.ts`, `ProjectileRuntime.ts`, `InfantryCombatState.ts`, `SimulationTick.ts`, `SceneExport.ts`, `package.json`.
...
Откат: automatic modes запрещаются, single остаётся.

## 17. Этап 9 — пулемёт и помощник
...
- четыре smoke wrapper/source files.

Изменить `FireTaskRuntime.ts`, `WeaponInstance.ts`, `InfantryCombatState.ts`, `SceneExport.ts`, `package.json`.

Deploy state фиксирует position/direction/traverse arc. Assistant ускоряет действия, но не обязателен. Потеря assistant не ломает weapon state. Нагрев и неисправности отсутствуют.

Откат: machine gun остаётся usable undeployed с табличным штрафом.

## 18. Этап 10 — secondary и ground equipment
...
Изменить `WeaponInstance.ts`, `InfantryCombatState.ts`, `SimulationTick.ts`, `SceneExport.ts`, `package.json`.
...
Откат: snapshots с ground equipment отклоняются явной migration error, а не молчаливой потерей.

## 19. Этап 11 — perception signals
...
Изменить `src/core/perception/PerceptionContact.ts`, `ShotCommitService.ts`, `ProjectileRuntime.ts`, `SimulationTick.ts`, `SceneExport.ts`, `package.json`.
...
Откат: combat signals отключаются, visual perception остаётся.

## 20. Этап 12A — core diagnostics
...
- два smoke wrapper/source files.

Изменить новые runtime-модули только для публикации уже рассчитанных counters/reasons, `src/core/debug/SimulationStepPerformanceDiagnostics.ts`, `package.json`.

UI не пересчитывает physics. Snapshot immutable и revision-driven.

Откат: удалить snapshots/counters; semantics не меняются.

## 21. Этап 12B — UI diagnostics и завершение редакторов
...
Откат: удалить panels/styles; runtime diagnostics остаются.

## 22. Этап 13 — Graph v2 integration
...
- четыре smoke wrapper/source files для commands и subjective facts.
...
Откат: удалить contracts/adapter; physics API остаётся callable tests/player.

## 23. Этап 14 — action-port integration
...
Откат: action-port combat task отключается; base observation/return остаётся.

## 24. Этап 15 — финальное переключение
...
## 25. Проверка результата исполнителя
...
14. для этапов 2–8, 13–15 и stage 4 назначает независимого verification-исполнителя;
15. запрашивает живую проверку только там, где tests не доказывают UX;
16. отдельно просит разрешение на перенос в preview.

## 26. Покрытие архитектуры

- Каталоги, revisions, loadouts, static editors: 1A–1B.
- Центральные channels и ownership: 2A–2B.
- FireTask, muzzle, risk, commitShot, exactly-once: 3.
- Fixed 1/30, physical projectiles, swept collision, broad phase, benchmark: 3–4.
