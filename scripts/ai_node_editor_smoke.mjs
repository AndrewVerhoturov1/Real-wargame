...
expectContains(main, 'createContractDefaultParameters', 'Значения по умолчанию должны браться из единого реестра контрактов.');
const contractRegistry = readText('src/core/ai/contracts/AiNodeContractRegistry.ts');
for (const needle of ["type: 'Reload'", "requiredParameter('durationSeconds'", "requiredParameter('targetAmmo'", "requiredParameter('failIfNoWeapon'"]) {
  expectContains(contractRegistry, needle, `Контракт Reload должен задавать параметр по умолчанию: ${needle}`);
}
...
const contractUi = readText('src/ai-node-editor/node-contract-ui.ts');
for (const needle of ['renderNodePorts', 'canConnectPorts', 'Нельзя передать', 'required-marker', 'describeNodeRu']) {
  expectContains(contractUi, needle, `Typed contract UI должен содержать: ${needle}`);
}
const subgraphUi = readText('src/ai-node-editor/subgraph-ui.ts');
for (const needle of ['listSubgraphChoices', 'cloneSubgraphGraph', 'renderGraphBreadcrumb', 'renderSubgraphSelect']) {
  expectContains(subgraphUi, needle, `Subgraph UI должен содержать: ${needle}`);
}
for (const needle of ['Проверить и обновить формат графа', 'migrateCurrentGraph', 'validateGraphLocally', 'data-validation-node-id', 'openSubgraphNode', 'closeCurrentSubgraph', 'data-typed-port-kind']) {
  expectContains(main, needle, `Graph v2 editor должен содержать: ${needle}`);
}
...
const nodeTypes = readText('src/core/ai/contracts/AiNodeContractRegistry.ts');
for (const needle of [
