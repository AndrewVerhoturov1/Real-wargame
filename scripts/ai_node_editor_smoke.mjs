...
expectContains(nodeTypes, 'Blackboard Threshold Condition', 'Универсальная нода должна иметь общий английский label.');
expectContains(nodeTypes, 'Параметр выше/ниже порога', 'Универсальная нода должна иметь русский overlay label выше/ниже.');
...
expectContains(humanUi, 'human-comparison-button', 'Универсальная нода должна иметь кнопки режима выше/ниже.');
expectContains(humanUi, 'Параметр ниже порога', 'В интерфейсе должна быть кнопка режима ниже порога.');
expectContains(humanUi, 'Value below threshold', 'В английском интерфейсе должна быть кнопка below threshold.');
expectContains(humanUi, 'comparison', 'Универсальная нода должна сохранять режим сравнения в parameters.comparison.');
expectContains(humanUi, 'compareThreshold', 'Human UI должен считать PASS/FAIL для above и below.');
...
expectContains(engineCore, 'readBranchThresholdCondition', 'Evaluate-once должен читать sourceKey/comparison/threshold из графа.');
expectContains(engineCore, 'evaluateThresholdCondition', 'Local engine должен считать above/below условие.');
expectContains(engineCore, 'parameters.comparison', 'Validation должен проверять parameters.comparison.');
