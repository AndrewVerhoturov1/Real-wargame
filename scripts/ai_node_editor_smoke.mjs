...
for (const needle of ['.human-node-panel', '.inspector-panel', '.app-shell-menu', '.combat-catalog-editor', 'select', 'input', 'textarea']) {
...
for (const needle of ['createContractDefaultParameters', 'renderContractParameterFields', 'readContractParameterFields']) {
  expectContains(main, needle, `Редактор должен получать параметры и поля из единого контракта: ${needle}`);
}
const contracts = readText('src/core/ai/contracts/AiNodeContractRegistry.ts');
for (const needle of ["type:'Reload'", "requiredParameter('durationSeconds'", "requiredParameter('targetAmmo'", "requiredParameter('failIfNoWeapon'"]) {
  expectContains(contracts, needle, `Контракт Reload должен содержать значение по умолчанию: ${needle}`);
}
...
const nodeTypes = readText('src/core/ai/AiNodeTypes.ts') + readText('src/core/ai/contracts/AiNodeContractRegistry.ts');
for (const needle of [
