...
async function run() {
  ...
  try {
    ...
    source = source.replace(
      ...
      '    assert.deepEqual(serializeInfantryCombatUnitRuntime(loaded.units[0]!.infantryCombatRuntime), serializeInfantryCombatUnitRuntime(original.state.units[0]!.infantryCombatRuntime), `${name}: unit combat runtime must restore exactly`);\n    continue;',
    );
...
async function runSmoke(sourceName, outputName) {
  ...
  await import(`${pathToFileURL(path.join(outDir, outputName)).href}?run=stage5-save-load-unit-immediate`);
}
