...
assertReliefProtectionIsNotDoubleCounted(east);
...
function assertReliefProtectionIsNotDoubleCounted(slope: ScenarioEvaluation) {
  const target = { x: REVERSE.x + 0.5, y: REVERSE.y + 0.5 };
  const threatPosition = { x: slope.threat.x, y: slope.threat.y };
  const fullCover = evaluateCoverBetween(
    slope.state.map,
    threatPosition,
    target,
    slope.blue.behaviorRuntime.posture,
  );
  const lineCoverWithoutRelief = evaluateCoverBetween(
    slope.state.map,
    threatPosition,
    target,
    slope.blue.behaviorRuntime.posture,
  ...
  const reverseCell = awarenessCell(slope.awareness, REVERSE);

  assert.ok(fullCover.protection > 0, 'the legacy/default cover contract must still detect reverse-slope relief');
  assert.equal(
    lineCoverWithoutRelief.protection,
    0,
    'a terrain-only ridge must contribute no object/forest line cover when relief is excluded',
  );
  assert.equal(
    reverseCell.expectedProtectionAgainstThreat,
    reverseCell.terrainProtection,
    'directional awareness must apply terrain relief exactly once when no object or forest lies on the threat line',
  );
  assert.equal(reverseCell.protectedAgainstThreatId, slope.threat.id);
}
