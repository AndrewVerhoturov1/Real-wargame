...
verifyLargeStepUsesOnlyPostureRemainder();
...
function verifyLargeStepUsesOnlyPostureRemainder(): void {
  const coarse = routeState('coarse-remainder');
  const split = routeState('split-remainder');

  tickSimulation(coarse, 0.7);
  tickSimulation(split, 0.45);
  tickSimulation(split, 0.25);

  assert.equal(coarse.units[0].behaviorRuntime.posture, 'crouched');
  assert.equal(split.units[0].behaviorRuntime.posture, 'crouched');
  assert.ok(
    Math.abs(coarse.units[0].position.x - split.units[0].position.x) < 1e-9,
    'a coarse step may translate only during the time remaining after posture completion',
  );
}
...
function routeState(id: string): SimulationState {
  const state = makeState({
    id,
    type: 'infantry_squad',
    side: 'blue',
  ...
  unit.order = createMoveOrder(
    { x: 40.5, y: unit.position.y },
    { source: 'player', ownerToken: `route-${id}` },
  );
  return state;
}

function makeState(unit: Parameters<typeof createInitialState>[1][number]): SimulationState {
  const state = createInitialState(mapData(), [unit]);
