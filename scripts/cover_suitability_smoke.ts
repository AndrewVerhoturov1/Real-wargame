...
function testFarMarginalCoverDoesNotBeatCloserOption(): void {
  ...
  assert.ok(result.bestQualityCoverCandidates.every((candidate) => candidate.x < 24), 'nearly equal far area should be dominated by closer cover');
}

function testIsolatedMinimumRejected(): void {
  const map = makeMap(10, 7);
  const danger = filled(10 * 7, 58);
...
function testDangerAndRouteInvalidation(): void {
  ...
  assert.notEqual(routeChanged.versions.routeCostFieldKey, dangerChanged.versions.routeCostFieldKey, 'route-cost profile change must invalidate route evaluation');
...
function makeFields(map: TacticalMap, danger: Uint8Array, passable = filledBytes(map.width * map.height, 1), key = 'synthetic'): RouteCostFields {
