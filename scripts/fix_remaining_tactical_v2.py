...
def replace_once(path: str, before: str, after: str) -> None:
    ...
    if before not in source:
        raise RuntimeError(f"Expected fragment not found in {path}: {before[:140]!r}")
...
def fix_typescript() -> None:
    path = "src/ai-node-editor/TacticalPositionProfileEditor.ts"
    replace_once(
        path,
    ...
    path = "src/core/tactical/TacticalPositionObjective.ts"
    replace_once(
        path,
...
def fix_tests() -> None:
    ...
    workspace_path = "scripts/tactical_workspace_smoke_incremental_directional.mjs"
    replace_once(
        workspace_path,
...
""",
    ...
    editor_path = "scripts/ai_node_editor_smoke.mjs"
    replace_once(
        editor_path,
