...
const customPerception = registry.replacePerceptionProfile({
  ...standardPerception,
  id: 'test-perception',
...
const standardCondition = resolveConditionProfileSnapshot('standard');
const customCondition = registry.replaceConditionProfile({
  ...standardCondition,
...
registry.replaceSoldierArchetype({
  ...
  builtIn: false,
  revision: 1,
  perceptionProfileId: customPerception.id,
  conditionProfileId: customCondition.id,
...
assert.equal(spawnedSoldier.archetypeId, 'test-archetype');
assert.equal(spawnedSoldier.perceptionProfileId, 'test-perception');
assert.equal(spawnedSoldier.conditionProfileId, 'runtime-condition');
...
assert.ok(Object.isFrozen(spawnedSoldier.perceptionProfile));
assert.ok(Object.isFrozen(spawnedSoldier.conditionProfile));
assert.deepEqual(spawnedSoldier.sourceProfileLinks, [
  { editorId: 'soldierArchetypes', profileId: 'test-archetype', labelRu: 'Архетип бойца' },
  { editorId: 'perceptionProfiles', profileId: 'test-perception', labelRu: 'Профиль восприятия' },
  { editorId: 'conditionProfiles', profileId: 'runtime-condition', labelRu: 'Ранения и подавление' },
]);
...
const observed = advanceVisualContact(null, {
  ...
  deltaSeconds: 1,
  nowSeconds: 1,
  perceptionProfile: spawnedSoldier.perceptionProfile,
});
assert.equal(observed.evidence, 40);
assert.equal(observed.confidence, 20, 'soldier snapshot must own confidence conversion');
assert.equal(observed.uncertaintyCells, 8, 'soldier snapshot must own uncertainty conversion');
...
const unit = createCapabilityTestUnit(spawnedSoldier);
...
assert.equal(clamped.wound.woundedMovementMultiplier, 1);
...
const builtInBefore = normalized.requireConditionProfile('standard');
const builtInReplacement = normalized.replaceConditionProfile({
  ...builtInBefore,
  wound: { ...builtInBefore.wound, woundedMovementMultiplier: 0.01 },
});
assert.equal(builtInReplacement.wound.woundedMovementMultiplier, 0.78);
assert.equal(normalized.requireConditionProfile('standard'), builtInBefore);
...
function createCapabilityTestUnit(soldier: SoldierParameters): UnitModel {
  return {
    id: 'test-unit',
    soldier,
    behaviorRuntime: {
