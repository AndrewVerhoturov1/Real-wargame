...
def replace_once(path: str, before: str, after: str) -> None:
    ...
    if before not in source:
        raise RuntimeError(f"Expected fragment not found in {path}: {before[:180]!r}")
...
replace_once('src/ai-node-editor/TacticalPositionProfileEditor.ts', '(draft.settings as Record<string, number | boolean>)[key] = input.checked;', '(draft.settings as unknown as Record<string, number | boolean>)[key] = input.checked;')
replace_once('src/ai-node-editor/TacticalPositionProfileEditor.ts', '(draft.settings as Record<string, number | boolean>)[key] = next;', '(draft.settings as unknown as Record<string, number | boolean>)[key] = next;')
replace_once('src/core/tactical/TacticalPositionObjective.ts', "import type { TacticalPositionSettings } from './TacticalPositionSettings';", "import type { TacticalPositionCandidateSeed } from '../ai/tactical/TacticalQuery';\nimport {\n  createDefaultTacticalPositionSettings,\n  type TacticalPositionSettings,\n} from './TacticalPositionSettings';")
replace_once('src/core/tactical/TacticalPositionObjective.ts', """  const requestedLimit = Math.max(1, Math.floor(request.maxCandidates));
...
replace_once('src/core/tactical/TacticalPositionObjective.ts', 'score: objectiveSortScore(enriched, request.objective, request.settings),', 'score: objectiveSortScore(enriched, request.objective, settings),')
replace_once('src/core/tactical/TacticalPositionObjective.ts', """export function readTacticalPositionObjectiveMetrics(
  candidate: TacticalPositionCandidateSeedV2,
): TacticalPositionObjectiveMetrics {
  const metrics = candidate.metrics as TacticalPositionCandidateSeedV2['metrics'] & Partial<TacticalPositionObjectiveMetrics>;""", """export function readTacticalPositionObjectiveMetrics(
...
replace_once('scripts/tactical_position_search_smoke.ts', "assert.equal(candidate.metrics.recommendedPosture, 'crouched');", "assert.equal(candidate.metrics.recommendedPosture, 'prone', 'a material prone safety advantage must produce a prone candidate');")

request_test = Path('scripts/tactical_position_request_service_smoke.ts')
request_source = request_test.read_text()
call_block = """verifyRendererDoesNotOwnTacticalSearch();
...
"""
if call_block not in request_source:
    raise RuntimeError('Request-service call block not found')
request_source = request_source.replace(call_block, '', 1)
request_test.write_text(request_source.rstrip() + '\n\n' + call_block)

workspace = 'scripts/tactical_workspace_smoke_incremental_directional.mjs'
replace_once(workspace, """  if (line.includes('src/tactical-workspace-stage8.css: missing')) {
    return line.includes('.cover-map-tooltip[hidden]');
  }
  return line.includes('src/core/knowledge/SoldierDangerField.ts: missing \"readDirectionalBasisValue\"');""", """  if (line.includes('src/tactical-workspace-stage8.css: missing')) {
    return line.includes('.cover-map-tooltip[hidden]');
  }
...
replace_between(workspace, 'function verifyWorkspaceMigration() {', 'function verifyOverlayMigration() {', """function verifyWorkspaceMigration() {
...
""")
replace_between(workspace, 'function verifySharedAwarenessRuntime() {', 'function verifyBoundedTacticalSearch() {', """function verifySharedAwarenessRuntime() {
...
if old_graph not in workspace_source:
    raise RuntimeError('Legacy graph runtime block not found')
workspace_file.write_text(workspace_source.replace(old_graph, new_graph, 1))

editor = 'scripts/ai_node_editor_smoke.mjs'
replace_once(editor, """  'src/core/ai/AiGraphRunner.ts',
  'src/data/ai/soldier_default_survival_graph.json',""", """  'src/core/ai/AiGraphRunner.ts',
...
replace_once(editor, """expectContains(html, '/src/ai-node-editor/runtime-debug-overlay.css', 'HTML должен подключать стили runtime debug overlay.');
expectContains(html, 'real-wargame.ai-node-editor.graph.v6', 'HTML должен bootstrap-ить новый чистый graph storage v6.');""", """expectContains(html, '/src/ai-node-editor/runtime-debug-overlay.css', 'HTML должен подключать стили runtime debug overlay.');
...
replace_once(editor, """expectNotContains(profileEditor, 'data-navigation-tab="diagnostics"', 'Отдельная вкладка Диагностика должна быть удалена.');
expectContains(main, 'addNodeFromPalette', 'Редактор должен уметь добавлять ноды из палитры.');""", """expectNotContains(profileEditor, 'data-navigation-tab="diagnostics"', 'Отдельная вкладка Диагностика должна быть удалена.');
...
replace_between(editor, "const graphRunner = readText('src/core/ai/AiGraphRunner.ts');", "const gameBridge = readText('src/core/ai/AiGameBridge.ts');", """const graphRunner = readText('src/core/ai/AiGraphRunner.ts');
