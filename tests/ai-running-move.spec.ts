...
test('shows running movement, route progress, and Russian authoring controls', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.addInitScript(({ graphKey, positionKey, uiKey, ui }) => {
    const graph = makeMovementGraph();
    ...
    function makeRuntimeDebug(graph: ReturnType<typeof makeMovementGraph>) {
      return {
        version: 1,
  ...
  await expect(panel).toContainText('Без прогресса');
  await expect(panel).toContainText('0.4 сек.');
...
test('shows blocked route and Russian abort reason', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.addInitScript(({ graphKey, positionKey, uiKey, ui }) => {
    const graph = {
      version: 1,
      id: 'blocked_route_browser_graph',
      name: 'Blocked Route Browser Graph',
      nameRu: 'Граф проверки заблокированного маршрута',
      rootNodeId: 'root',
      blackboardDefaults: {},
      nodes: [
        { id: 'root', type: 'Root', displayName: 'Start', displayNameRu: 'Старт', children: ['move'], parameters: {} },
        {
          id: 'move',
          type: 'MoveToBlackboardPosition',
          displayName: 'Move to cover',
    ...
    localStorage.setItem('real-wargame.ai-node-editor.debug.v1', JSON.stringify({
      ...
      kind: 'ai-graph-runtime-debug',
      graphId: graph.id,
      unitId: 'soldier_blocked_test',
      unitLabel: 'Стрелок 1',
      selectedBranchNodeId: 'move',
      selectedBranchName: 'Move to cover',
      selectedBranchNameRu: 'Двигаться к укрытию',
      ok: false,
      status: 'cancelled',
      activeNodeId: 'move',
      activeNodeName: 'Move to cover',
      ...
      targetPosition: { x: 18.5, y: 12.5 },
      distanceRemainingCells: 7.4,
      actionToken: 'soldier_blocked_test:move:0',
      routeStatus: 'blocked',
      ...
      previewOnly: false,
      nowMs: Date.now(),
      simulationNowMs: 2800,
      explanation: 'Route blocked.',
      explanationRu: 'Маршрут заблокирован.',
      trace: [{ nodeId: 'move', nodeType: 'MoveToBlackboardPosition', status: 'cancelled', reason: 'Route blocked.', reasonRu: 'Маршрут заблокирован.' }],
      scores: [],
      effects: [],
      blackboard: {},
      lifecycle: [{ phase: 'cancel', nodeId: 'move', nodeType: 'MoveToBlackboardPosition', atMs: 2800, reason: 'Route blocked.', reasonRu: 'Маршрут заблокирован.' }],
    }));
  }, {
    graphKey: GRAPH_STORAGE_KEY,
    positionKey: POSITION_STORAGE_KEY,
  ...
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, '28-ai-route-blocked.png') });
});
