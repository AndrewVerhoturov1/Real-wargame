...
test('deployed Soviet soldier prototype passes exact-SHA visual QA', async ({ page, request }) => {
  ...
  page.on('console', (message) => {
    if (message.type() === 'error') {
      ...
      if (text.includes('vercel.live') || text.includes('_next-live')) evidence.ignoredServiceFailures.push(`console: ${text}`);
  ...
  page.on('requestfailed', (failed) => {
    ...
    if (failed.url().includes('vercel.live') || failed.url().includes('_next-live')) evidence.ignoredServiceFailures.push(text);
  ...
  try {
    ...
    await chooseSize(page, 32);
    await choosePreset(page, 'split', /Разные направления/);
