...
test('final exact Polygon shell visual contract after reference correction', async ({ page, request }) => {
  ...
  page.on('requestfailed', (failedRequest) => {
    ...
    const ignored = url.includes('vercel.live')
      || url.includes('_next-live')
      || url.includes('/.well-known/vercel/')
      || (failedRequest.method() === 'HEAD' && url === targetUrl);
    if (ignored) evidence.ignoredServiceFailures.push(entry);
  ...
  try {
    ...
    evidence.observedProductSha = identity.sourceSha;
    ...
    expect(await page.locator('.polygon-shell-map-placeholder').evaluate((element) => getComputedStyle(element).pointerEvents)).toBe('none');

    await expect(page.locator('.polygon-shell-topbar-center .polygon-shell-top-button')).toHaveText(['ФАЙЛ', 'РЕДАКТОРЫ']);
    await expect(page.locator('.polygon-shell-topbar-right .polygon-shell-top-button')).toHaveText(['ВИД ▾', 'EN']);
    await expect(page.locator('.app-shell-menu-trigger')).toBeVisible();
    expect(await page.locator('.app-shell-menu-trigger').evaluate((element) => getComputedStyle(element, '::before').content)).toContain('МЕНЮ');
    ...
    near(leftTabs[3]!.y - leftTabs[0]!.y, 34, 1);
    ...
    for (let index = 1; index < runBoxes.length; index += 1) expect(runBoxes[index]!.x).toBeGreaterThan(runBoxes[index - 1]!.x);
    ...
    await page.screenshot({ path: `${artifactDir}/01-final2-shell-1600x900.png`, fullPage: false });
    ...
    await page.screenshot({ path: `${artifactDir}/02-final2-tabs-selected-1600x900.png`, fullPage: false });
    ...
    await page.screenshot({ path: `${artifactDir}/03-final2-panels-collapsed-1600x900.png`, fullPage: false });
    ...
    expect(await page.locator('.polygon-shell-topbar-center').evaluate((element) => getComputedStyle(element).display)).toBe('none');
    await expect(duration).toBeVisible();
    await expect(seed).toBeVisible();
    (evidence.geometry as Record<string, unknown>).narrow = { left: narrowLeft, right: narrowRight, board: narrowBoard };
    await page.screenshot({ path: `${artifactDir}/04-final2-shell-1080x800.png`, fullPage: false });
