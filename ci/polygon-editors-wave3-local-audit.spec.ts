...
async function parityInvariants(host: ReturnType<Page['locator']>, editorId: string): Promise<Record<string, unknown>> {
  const result = await host.evaluate((node, id) => {
    const hidden = [...node.querySelectorAll<HTMLElement>('.polygon-editor-parity-root [hidden]')];
    const hiddenViolations = hidden
    ...
    const soldierTabs = id === 'soldierArchetypes'
      ...
      : null;
    const flow = id === 'perceptionProfiles'
      ? node.querySelector<HTMLElement>('.polygon-perception-flow')
      : null;
  ...
  if (editorId === 'soldierArchetypes') {
    const tabs = result.soldierTabs as { display: string; visibility: string; height: number } | null;
    expect(tabs).not.toBeNull();
    expect(tabs?.display).not.toBe('none');
    expect(tabs?.visibility).not.toBe('hidden');
    expect(tabs?.height ?? 0).toBeGreaterThanOrEqual(30);
  }
  if (editorId === 'perceptionProfiles') {
    const flow = result.perceptionFlow as { height: number } | null;
    expect(flow).not.toBeNull();
    expect(flow?.height ?? 999).toBeLessThanOrEqual(100);
  }
  return result;
}

test('captures all 11 exact Polygon editor states and wave 3 invariants', async ({ browser }) => {
  ...
  const saveEvidence = () => fs.writeFileSync(path.join(artifactRoot, 'wave3-local-audit-evidence.json'), JSON.stringify(evidence, null, 2));
  ...
  try {
    ...
    for (let index = 0; index < editors.length; index += 1) {
      ...
      if (editor.id === 'surfaceTypes') {
        ...
        const screenshot = `wave3-product-${number}-${editor.slug}-unavailable.png`;
      ...
      const invariants = await parityInvariants(host, editor.id);
      const screenshot = `wave3-product-${number}-${editor.slug}.png`;
      await portal.screenshot({ path: path.join(artifactRoot, screenshot), animations: 'disabled' });
      (evidence.editors as Array<Record<string, unknown>>).push({ ...editor, disabled, title, screenshot, state: 'active', invariants });
      saveEvidence();
    }
...
test('captures aligned Linear infantryman state', async ({ page }) => {
  ...
  const host = portal.locator('.polygon-global-editor--soldierArchetypes');
  await parityInvariants(host, 'soldierArchetypes');
  await portal.screenshot({ path: path.join(artifactRoot, 'wave3-product-03-soldier-archetypes-aligned.png'), animations: 'disabled' });
});
...
test('captures aligned PPSh-41 weapon state', async ({ page }) => {
  ...
  await expect(target).toBeVisible();
  await target.click();
  const host = portal.locator('.polygon-global-editor--weapons');
  await parityInvariants(host, 'weapons');
  await portal.screenshot({ path: path.join(artifactRoot, 'wave3-product-07-weapons-aligned.png'), animations: 'disabled' });
});
