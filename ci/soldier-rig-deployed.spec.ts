...
const outputDir = path.resolve('artifacts/soldier-rig-deployed');
...
function isVercelService(text: string): boolean {
  return text.includes('vercel.live') || text.includes('_next-live') || text.includes('/.well-known/vercel/');
}
...
test('deployed transferred soldier rig passes exact-SHA visual QA', async ({ page, request }) => {
  ...
  try {
    ...
    evidence.productShaMatch = source.ref === featureBranch && evidence.observedProductSha === expectedSha;
    ...
    evidence.stage = 'exact-deployment-loaded';
