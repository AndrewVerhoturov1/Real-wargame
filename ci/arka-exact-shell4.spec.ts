...
test('final prototype-matched Polygon shell', async ({ page, request }) => {
  ...
  try {
    ...
    await expect(page.locator('.polygon-shell-topbar-center')).toContainText('ФАЙЛ');
    await expect(page.locator('.polygon-shell-topbar-center')).toContainText('РЕДАКТОРЫ');
    await expect(page.locator('.polygon-shell-topbar-right')).toContainText('ВИД');
    await expect(page.locator('.polygon-shell-topbar-right')).toContainText('EN');
    ...
    near(nl!.x,14,2); near(nl!.width,372,2); near(nr!.x,730,2); near(nr!.width,336,2); near(nb!.x,431,3); near(nb!.y,317,3); near(nb!.width,254,3); near(nb!.height,254,3);
