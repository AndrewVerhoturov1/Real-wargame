...
import { installUnitBarPresentation } from './UnitBarPresentation';
import {
...
export function installTacticalWorkspace(
  ...
  const cleanupLegacy = installLegacyTacticalWorkspace(state, aiBridge, onChanged);
  const cleanupPresentation = installUnitBarPresentation(state);
