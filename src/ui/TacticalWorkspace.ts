...
export function installTacticalWorkspace(
  ...
  shell.innerHTML = `
    ...
    <section class="simulation-unit-bar">
      ...
      <div class="unit-bar-route-controls">
        <div class="tactical-overlay-segmented" role="group" aria-label="Режим тактического слоя">${OVERLAY_MODES.map(([id, label]) => `<button type="button" data-overlay-mode="${id}" title="Горячая клавиша V">${label}</button>`).join('')}</div>
  ...
  function renderSidebar(): void {
    ...
    if (!unit) {
      ...
      sidebarBody.innerHTML = infoPanel(state, unit);
    } else if (tab === 'danger') {
      renderDanger(sidebarBody, state, unit, onChanged);
    } else if (tab === 'stealth') {
      sidebarBody.innerHTML = stealthPanel(state, unit);
    } else {
  ...
  const interval = window.setInterval(() => {
    if (mode === 'simulation' && !document.hidden) update(false);
  }, 300);
...
function renderDanger(target: HTMLElement, state: SimulationState, unit: UnitModel, onChanged: () => void): void {
  ...
  target.innerHTML = `${heading('Единый тактический слой','Опасность — каноническое цветное поле. Укрытия — нейтральные достижимые области, рассчитанные по danger и route-cost fields.')}
    ${grid([
      ['Текущая опасность', pct(threats.danger)],
...
function infoPanel(state: SimulationState, unit: UnitModel): string {
  ...
  return `${heading('Инфо','На этой вкладке тактический overlay скрыт.')}${grid([
    ['Положение', `${unit.position.x.toFixed(1)}, ${unit.position.y.toFixed(1)}`],
    ['Высота', cell ? elevation(sampleSmoothHeightLevel(state.map, unit.position.x, unit.position.y)) : 'вне карты'],
    ['Местность', cell ? terrain(cell) : 'вне карты'],
    ['Поза', postureLabel(unit.behaviorRuntime.posture)],
    ['Действие', actionLabel(unit.behaviorRuntime.currentAction)],
    ['Приказ', orderLabel(state, unit)],
    ['Решение ИИ', unit.behaviorRuntime.aiGraphReason || unit.behaviorRuntime.reason || 'ещё не рассчитывалось'],
  ])}`;
}

function stealthPanel(state: SimulationState, unit: UnitModel): string {
  const cell = getCell(state.map, Math.floor(unit.position.x), Math.floor(unit.position.y));
  return `${heading('Слой скрытности','Существующая карта скрытности сохранена; поиск укрытий больше не выполняет отдельные LOS-проверки.')}${grid([
    ['Поза', postureLabel(unit.behaviorRuntime.posture)],
    ['Тип клетки', cell ? terrain(cell) : 'вне карты'],
    ['Известных угроз', String(unit.tacticalKnowledge.threats.length)],
  ])}`;
}
...
function memoryPanel(state: SimulationState, unit: UnitModel): string {
  ...
  return `${heading('Обзор и память','Укрытия не хранятся как старые предметные метки: они выводятся из актуальных канонических полей.')}${grid([
    ['Угроз в памяти', String(unit.tacticalKnowledge.threats.length)],
    ['Версия знаний', String(unit.tacticalKnowledge.revision)],
  ])}<section class="workspace-panel-section"><h3>Опасности и противник</h3>${threats}</section>`;
}
...
function buildWorkspaceKey(state: SimulationState, mode: TacticalWorkspaceMode, tab: SimulationTab, collapsed: boolean): string {
  ...
  return [
    mode, tab, collapsed ? 1 : 0, getTacticalOverlayMode(state), unit?.id ?? 'none',
    unit?.position.x.toFixed(2) ?? '', unit?.position.y.toFixed(2) ?? '',
    unit?.behaviorRuntime.posture ?? '', unit?.behaviorRuntime.currentAction ?? '',
    unit?.tacticalKnowledge.revision ?? 0, unit?.order?.target.x.toFixed(2) ?? '', unit?.order?.target.y.toFixed(2) ?? '',
    revisions.terrain, revisions.height, revisions.forest, revisions.objects,
  ].join('|');
}
...
function terrain(cell: MapCell): string {
  const labels: Record<MapCell['terrain'], string> = { road:'дорога', field:'поле', rough:'пересечённая местность', swamp:'болото', water:'вода' };
