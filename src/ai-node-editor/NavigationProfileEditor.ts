...
const groups: ReadonlyArray<{ title: string; fields: readonly NumericDefinition[] }> = [
  ...
  },
  {
    ...
    fields: [
      ...
      numeric('exposureWeight', 'Избегание видимости противнику', 'Пока честные субъективные данные видимости недоступны, фактор равен нулю.', 0, 5, 0.05, 'вес'),
      ...
      numeric('enemyDistanceWeight', 'Отношение к близости противника', 'Точная субъективная дистанция пока не используется.', -3, 3, 0.05, 'вес'),
    ],
  },
  {
    title: 'Территория',
    fields: [
      numeric('territoryWeights.friendly', 'Своя территория', 'Будущий штраф или бонус своей территории.', -3, 3, 0.05, 'вес'),
  ...
  },
  {
    ...
    fields: [
      numeric('maximumDetourRatio', 'Максимальный обход', '1,6 означает, что обход может быть длиннее на 60%.', 1, 3, 0.05, 'отношение'),
      numeric('maximumRouteCost', 'Максимальная цена маршрута', 'Пустое поле отключает предел.', 1, 100000, 1, 'цена'),
    ],
  },
  {
    title: 'Перестроение маршрута',
    fields: [
      numeric('replanRules.minimumCostImprovement', 'Минимальное улучшение', 'Новый путь принимается только при достаточном улучшении.', 0, 1, 0.01, 'доля'),
      numeric('replanRules.minimumDangerRevisionInterval', 'Интервал ревизий опасности', 'Защита от перестроения при каждом малом обновлении знаний.', 1, 20, 1, 'ревизий'),
...
export function mountNavigationProfileEditor(context: GameEditorMountContext): GameEditorInstallation {
  ...
  let selectedProfileId = context.request.profileId && registry.hasProfile(context.request.profileId)
    ...
    : registry.listProfiles()[0]?.id ?? 'normal';
  let draft = registry.getProfile(selectedProfileId);
  let baseline = serializeProfile(draft);
  let destroyed = false;
  ...
  return {
    beforeClose(): boolean {
      ...
      return window.confirm('Отменить несохранённые изменения профиля маршрута и закрыть редактор?');
    },
    destroy(): void {
      if (destroyed) return;
      destroyed = true;
      ...
      delete panel.dataset.activeProfileEditor;
    },
  };

  function isDirty(): boolean {
    return serializeProfile(draft) !== baseline;
  }

  function resetDraft(): void {
    draft = registry.getProfile(selectedProfileId);
    baseline = serializeProfile(draft);
  }

  function render(): void {
    panel.dataset.activeProfileEditor = 'routeProfiles';
    panel.innerHTML = `
      <div class="navigation-profile-layout">
        <aside class="navigation-profile-list-panel">
          <div class="navigation-profile-list-heading">
            <div><h2>Профили маршрута</h2><p>Постоянные правила оценки пути. Это не ноды ИИ.</p></div>
        ...
        <main class="navigation-profile-form-panel">
          ...
          ${groups.map((group) => `
            <section class="navigation-profile-group">
              <h3>${group.title}</h3>
              <div class="navigation-profile-field-grid">${group.fields.map(renderNumericField).join('')}</div>
            </section>
          `).join('')}
          <section class="navigation-profile-group">
            ...
            <div class="navigation-profile-checkbox-grid">
              ...
              ${checkbox('replanRules.replanOnBlocked', 'Перестраивать при блокировке', 'Искать новый путь при блокировке ближайших клеток.')}
              ${checkbox('replanRules.replanOnProfileChange', 'Перестраивать при изменении профиля', 'Применять сохранённые изменения к активному маршруту.')}
              ${checkbox('replanRules.replanOnDangerChange', 'Перестраивать при изменении опасности', 'Учитывать новые знания после интервала ревизий.')}
            </div>
          </section>
        </main>
      </div>`;
  ...
  function bind(): void {
    ...
    panel.querySelectorAll<HTMLButtonElement>('[data-reset-field]').forEach((button) => {
      button.addEventListener('click', () => {
        setDraftPath(button.dataset.resetField ?? '', getDefaultValue(button.dataset.resetField ?? ''));
        render();
      });
    });
    ...
    panel.querySelector<HTMLInputElement>('[data-profile-import]')?.addEventListener('change', (event) => void importRegistry(event));
  }
  ...
  function handleAction(action: string): void {
    ...
    if (action === 'import') panel.querySelector<HTMLInputElement>('[data-profile-import]')?.click();
  }
...
export function mountSoldierDataEditor(context: GameEditorMountContext): GameEditorInstallation {
  const panel = context.host;
  const raw = window.localStorage.getItem('real-wargame.ai-node-editor.graph.v6');
  let defaults: unknown = {};
  try {
    defaults = raw ? (JSON.parse(raw) as { blackboardDefaults?: unknown }).blackboardDefaults ?? {} : {};
  } catch {
    defaults = {};
  }
  panel.dataset.activeProfileEditor = 'soldierData';
  panel.innerHTML = `
    <section class="navigation-profile-placeholder">
      ...
      ${context.request.selectedUnitId ? `<p>Выбранный боец: <code>${escapeHtml(context.request.selectedUnitId)}</code></p>` : ''}
...
function serializeProfile(profile: NavigationProfile): string {
  return JSON.stringify(profile);
}

function extremeText(value: number | null, definition: NumericDefinition): string {
  if (value === null) return '';
  const edge = (definition.max - definition.min) * 0.08;
