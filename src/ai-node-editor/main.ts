...
function renderGraphBreadcrumb(): string {
  const parts = ['Главный граф', ...graphNavigation.map((entry) => entry.labelRu), ...(graphNavigation.length ? [editorGraph.nameRu ?? editorGraph.name] : [])];
...
function saveSelectedNodeFromInspector(): void {
  ...
  try {
    ...
    const selectedSubgraph=document.querySelector<HTMLSelectElement>('#inspector-subgraph-id')?.value;
    if(node.type==='Subgraph'&&selectedSubgraph){contractParameters.subgraphId=selectedSubgraph;contractParameters.cancelPolicy='cancel_child';}
