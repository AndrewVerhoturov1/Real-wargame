  ...
  panel.innerHTML = `
    <div class="navigation-profile-layout environment-profile-layout">
      <aside class="navigation-profile-list-panel">
        ...
        <div class="navigation-profile-list">${Object.values(materials).map((item) => `<button type="button" data-environment-material="${attr(item.id)}" class="${item.id === selectedMaterialId ? 'active' : ''}"><strong>${html(item.nameRu)}</strong></button>`).join('')}</div>
      </aside>
...
function renderVegetation(material: VegetationMaterialDefinition): string {
  return [
    section('Материал', [
      textField('nameRu', 'Название', material.nameRu, 'Понятное название материала в интерфейсе и диагностике.'),
    ]),
    section('Внешний вид', [
      colorField('presentation.colorTint', 'Цветовой оттенок', material.presentation.colorTint, 'Цвет непрерывной растровой текстуры.'),
      selectField('presentation.textureId', 'Рисунок текстуры', material.presentation.textureId, [
        ['procedural_forest', 'Естественный лес'],
        ['procedural_forest_soft', 'Мягкий лесной покров'],
        ['procedural_forest_rough', 'Контрастный лесной покров'],
        ['none', 'Без текстурного рисунка'],
      ], 'Визуальный рисунок непрерывного слоя. На игровые свойства не влияет.'),
...
function renderSurface(material: SurfaceMaterialDefinition): string {
  return [
    section('Материал', [
      textField('nameRu', 'Название', material.nameRu, 'Понятное название поверхности в интерфейсе карты.'),
    ]),
    section('Внешний вид', [
...
function bind(panel: HTMLElement): void {
  ...
  panel.querySelectorAll<HTMLInputElement | HTMLSelectElement>('[data-environment-path]').forEach((input) => {
    const eventName = input instanceof HTMLInputElement && input.type === 'text' ? 'change' : 'input';
    input.addEventListener(eventName, () => queueMaterialUpdate(panel, input));
  });
...
function queueMaterialUpdate(panel: HTMLElement, input: HTMLInputElement | HTMLSelectElement): void {
...
function handleAction(panel: HTMLElement, action: string): void {
  if (action === 'create') {
    const source = registry.getProfile(selectedProfileId);
    const nameRu = window.prompt('Название нового профиля:', `${source.nameRu} — копия`);
    if (!nameRu) return;
    const id = createUniqueProfileId(nameRu);
    try {
      const created = registry.createCustomProfile(id, `${source.nameEn} copy`, nameRu, selectedProfileId);
    ...
    const current = registry.getProfile(selectedProfileId);
    const nameRu = window.prompt('Новое название профиля:', current.nameRu);
    if (!nameRu) return;
    registry.renameProfile(selectedProfileId, current.nameEn, nameRu);
    saveEnvironmentProfileRegistry(registry);
    renderEnvironmentProfiles(panel);
...
function ensureSubscription(panel: HTMLElement): void {
  ...
  subscribed = true;
  unsubscribe = subscribeEnvironmentProfileRegistry((next) => {
    registry = next;
    selectedProfileId = registry.hasProfile(selectedProfileId) ? selectedProfileId : registry.activeProfileId;
    if (!panel.hidden) renderEnvironmentProfiles(panel);
  });
}

export async function requestEnvironmentProfileEditorLeave(): Promise<boolean> {
  if (saveTimer !== null) await new Promise<void>((resolve) => window.setTimeout(resolve, 100));
  return true;
}

export function disposeEnvironmentProfileEditorPanel(): void {
  if (saveTimer !== null) window.clearTimeout(saveTimer);
  saveTimer = null;
  unsubscribe?.();
  unsubscribe = null;
...
function selectField(path: string, label: string, value: string, options: ReadonlyArray<readonly [string, string]>, help: string): string {
  const values = options.some(([option]) => option === value) ? options : [[value, 'Импортированный рисунок'] as const, ...options];
  return `<article class="navigation-profile-field"><div class="navigation-profile-field-title"><strong>${label}</strong></div><p>${help}</p><select data-environment-path="${attr(path)}">${values.map(([option, title]) => `<option value="${attr(option)}" ${option === value ? 'selected' : ''}>${html(title)}</option>`).join('')}</select></article>`;
}
function textField(path: string, label: string, value: string, help: string): string { return `<article class="navigation-profile-field"><div class="navigation-profile-field-title"><strong>${label}</strong></div><p>${help}</p><input type="text" data-environment-path="${attr(path)}" value="${attr(value)}" maxlength="80"/></article>`; }
...
function createUniqueProfileId(nameRu: string): string {
  const base = transliterate(nameRu).toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '') || 'environment';
  let candidate = base;
  let suffix = 2;
  while (registry.hasProfile(candidate) || candidate === 'default') candidate = `${base}_${suffix++}`;
  return candidate;
}
function transliterate(value: string): string {
  ...
  return [...value].map((char) => letters[char.toLowerCase()] ?? char).join('');
}
