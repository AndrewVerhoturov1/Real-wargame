...
export interface CombatLabLiveUnitSnapshotV1 {
  ...
  readonly playerOrderStateRu: string;
  readonly playerOrderLabelRu: string;
  readonly playerOrderDetailRu: string;
  ...
  readonly woundSummaryRu: string;
  readonly bloodLoss: number;
  readonly firstAidCharges: number;
  readonly facingDegrees: number;
  readonly attentionProfileId: string | null;
  readonly movementProfileId: string | null;
...
export function buildCombatLabLiveUnitSnapshot(
  ...
  const order = resolvePlayerOrder(unit);
  const snapshot = {
    unitId: unit.id,
    ...
    playerOrderStateRu: order.stateRu,
    playerOrderLabelRu: order.labelRu,
    playerOrderDetailRu: order.detailRu,
    currentAction,
    weapon,
    weaponReadiness,
    wounds,
    woundSummaryRu: woundSummaryRu(wounds),
    bloodLoss: combat.physiology.blood.bloodLoss,
    firstAidCharges: combat.medical.firstAidCharges,
    facingDegrees: normalizeDegrees(unit.facingRadians * (180 / Math.PI)),
    attentionProfileId: cleanId(unit.playerAttentionProfileId),
    movementProfileId: cleanId(
...
function resolveWeaponReadiness(unit: UnitModel): CombatLabWeaponReadinessV1 {
  ...
  if (fireTask && !isTerminalFirePhase(fireTask.phase)) {
    return readiness(
      'engaged',
      fireTask.phase === 'firing' ? 'Ведёт огонь' : 'Занято огневой задачей',
      fireTask.resultRu ?? 'Выполняется штатная огневая задача.',
    );
  }
...
function resolveCurrentAction(unit: UnitModel): CombatLabLiveUnitCurrentActionV1 {
  ...
  if (posture?.status === 'running') {
    return action(
      'posture_transition',
      `Меняет позу → ${postureLabelRu(posture.targetPosture)}`,
      posture.reasonRu || null,
    );
  }
  ...
  if (reload) {
    return action(
      'reload',
      reload.status === 'waiting_for_locomotion' ? 'Ждёт остановки для перезарядки' : 'Перезаряжается',
      `Этап: ${reload.stageId}`,
    );
  }
...
function resolvePlayerOrder(unit: UnitModel): {
  readonly stateRu: string;
  readonly labelRu: string;
  ...
  if (!command) {
    return Object.freeze({
      stateRu: 'Нет',
      labelRu: 'Без приказа',
      detailRu: 'Игрок не задавал отдельной команды.',
    });
  }
  ...
  const preset = command.intent.presetId === 'assault'
    ...
    : command.intent.presetId === 'recon'
      ? 'Разведка'
      : 'Движение';
  const stateRu = command.status === 'active'
    ? 'Активный'
  ...
  return Object.freeze({
    stateRu,
    labelRu: preset,
    detailRu: command.reasonRu || `Цель: ${Math.round(command.target.x)} × ${Math.round(command.target.y)}`,
  });
}
...
function capabilityLabelRu(unit: UnitModel, alive: boolean, conscious: boolean): string {
  ...
  if (unit.behaviorRuntime.suppression >= 65) return 'Сильно подавлен';
  if (unit.behaviorRuntime.suppression >= 35) return 'Подавлен';
  return 'Боеспособен';
}

function woundSummaryRu(wounds: readonly CombatLabLiveUnitWoundV1[]): string {
  if (wounds.length === 0) return 'Ранений нет';
  const severe = wounds.filter((wound) => wound.severity === 'critical' || wound.severity === 'severe').length;
  if (severe > 0) return `${wounds.length} ран. · тяжёлых ${severe}`;
  return `${wounds.length} ран.`;
}

function readiness(
  kind: CombatLabWeaponReadinessKind,
  labelRu: string,
  reasonRu: string,
): CombatLabWeaponReadinessV1 {
  return Object.freeze({ kind, labelRu, reasonRu });
}

function action(
  kind: CombatLabLiveUnitCurrentActionKind,
  labelRu: string,
  detailRu: string | null,
): CombatLabLiveUnitCurrentActionV1 {
...
function presentationKey(snapshot: Omit<CombatLabLiveUnitSnapshotV1, 'presentationKey'>): string {
  return JSON.stringify({
    ...
    order: [snapshot.playerOrderStateRu, snapshot.playerOrderLabelRu, snapshot.playerOrderDetailRu],
    ...
    attentionProfileId: snapshot.attentionProfileId,
    movementProfileId: snapshot.movementProfileId,
