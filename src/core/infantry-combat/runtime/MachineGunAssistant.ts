...
export function validateMachineGunAssistant(
  ...
  if (helper.side !== gunner.side) return invalid('assistant_wrong_side', 'Помощник должен быть на той же стороне.');
  if (gunner.infantryCombatRuntime.ammoInventory.role !== 'machine_gunner') return invalid('assistant_target_role_invalid', 'Получатель помощи не является пулемётчиком.');
  if (helper.infantryCombatRuntime.ammoInventory.role !== 'assistant_machine_gunner') return invalid('assistant_role_invalid', 'Указанный боец не является помощником пулемётчика.');
  const distance = Math.hypot(helper.position.x - gunner.position.x, helper.position.y - gunner.position.y) * state.map.metersPerCell;
  if (distance > MACHINE_GUN_ASSISTANT_MAX_DISTANCE_METRES + 1e-9) return invalid('assistant_too_far', 'Помощник находится слишком далеко.');
...
function invalid(reasonCode: string, reasonRu: string): MachineGunAssistantValidationV1 {
