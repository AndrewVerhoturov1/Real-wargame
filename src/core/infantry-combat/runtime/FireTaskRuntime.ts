...
export function requestSingleFireTask(
  ...
  if (input.mode !== undefined && input.mode !== 'single') {
    return requestRejected('unsupported_mode', 'infantry_fire_task_unsupported_mode', 'Stage 3 поддерживает только одиночный выстрел.');
  }
...
export function tickFireTaskWithTimeBudget(
  ...
  if (task.phase === 'accepted') transition(task, 'weapon_ready', intervalStartSeconds + consumedSeconds);
  if (task.phase === 'aiming' && task.aimQuality + TIME_EPSILON_SECONDS >= task.minimumSolutionQuality) {
    task.aimQuality = Math.max(task.aimQuality, task.minimumSolutionQuality);
    transition(task, 'firing', intervalStartSeconds + consumedSeconds);
    return tickResult(task.taskId, true, false, false, consumedSeconds, remainingSeconds, null);
  }
...
export function normalizeFireTaskRuntime(value: unknown): FireTaskRuntimeV1 | null {
  ...
  return {
    ...
    mode: 'single',
    phase,
    requestedSeconds: finiteNonNegative(value.requestedSeconds, 0),
    ...
    aimQuality: clamp01(value.aimQuality),
...
function completeActiveFireTask(unit: UnitModel, endedSeconds: number): void {
  ...
  if (task.actionHandle) {
    completePhysicalAction(unit, task.actionHandle, { endedSeconds, resultCode, resultRu });
  }
...
function normalizeActivePhase(value: unknown): FireTaskRuntimeV1['phase'] | null {
  return value === 'accepted' || value === 'weapon_ready' || value === 'aiming' || value === 'firing' || value === 'recovery'
    ? value
    : null;
}
...
function normalizePoint(value: unknown): BallisticPoint3 | null {
  if (!isRecord(value)) return null;
  if (!isFiniteNumber(value.xMetres) || !isFiniteNumber(value.yMetres) || !isFiniteNumber(value.zMetres)) return null;
