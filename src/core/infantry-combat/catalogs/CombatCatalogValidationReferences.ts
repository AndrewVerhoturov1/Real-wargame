...
export function validateReferences(ammo: unknown[], weapons: unknown[], loadouts: unknown[], issues: Issues): void {
  const ammoRefs = new Map<string, Record<string, unknown>>();
  const ammoIds = new Set<string>();
  for (const raw of ammo) if (isObject(raw) && validIdRevision(raw, 'ammoDefinitionId')) {
    ammoRefs.set(refKey(raw.ammoDefinitionId as string, raw.revision as number), raw);
  ...
  for (const raw of weapons) if (isObject(raw) && validIdRevision(raw, 'weaponDefinitionId')) {
    ...
    const ammoTarget = ref ? ammoRefs.get(refKey(ref.definitionId, ref.revision)) : undefined;
    if (ref && !ammoTarget) {
      add(issues, `${path}.ammo`, 'missing_ammo_reference', 'Оружие ссылается на отсутствующую точную ревизию боеприпаса.');
    } else if (ammoTarget && isStableStatus(raw.status) && ammoTarget.status === 'draft') {
      add(issues, `${path}.ammo`, 'unstable_ammo_reference', 'Опубликованное или архивное оружие не может ссылаться на draft-ревизию боеприпаса.');
    }
  ...
  for (const raw of loadouts) if (isObject(raw) && validIdRevision(raw, 'loadoutTemplateId')) {
    ...
    const stableSource = isStableStatus(raw.status);
    checkLoadoutWeapon(raw.primary, `${path}.primary`, weaponRefs, stableSource, issues);
    if (raw.secondary !== null) checkLoadoutWeapon(raw.secondary, `${path}.secondary`, weaponRefs, stableSource, issues);
...
function checkLoadoutWeapon(
  value: unknown, path: string, weapons: Map<string, Record<string, unknown>>, stableSource: boolean, issues: Issues,
): void {
  ...
  if (stableSource && weapon.status === 'draft') {
    add(issues, `${path}.definition`, 'unstable_weapon_reference', 'Опубликованный или архивный шаблон не может ссылаться на draft-ревизию оружия.');
  }
...
function isStableStatus(value: unknown): boolean {
