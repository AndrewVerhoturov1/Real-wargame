...
export type AiInputBinding =
  ...
  | {
      readonly source: 'subgraphInput';
      readonly port: string;
    };

export type AiOutputBinding =
  | {
      readonly target: 'blackboard';
      readonly key: string;
    }
  | {
      readonly target: 'subgraphOutput';
      readonly port: string;
    };
...
export function areAiPortKindsCompatible(
  outputKind: AiPortValueKind,
  inputKind: AiPortValueKind,
): boolean {
  if (outputKind === inputKind) return true;
  // Object identities intentionally stay strict. A unit is not a generic object
  // because using it as a position or slot would hide authoring mistakes.
  return false;
}
...
function isSerializableLiteral(value: unknown): value is AiBlackboardValue {
  return value === null
    || typeof value === 'string'
