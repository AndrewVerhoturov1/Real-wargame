...
export interface AiParameterDefinition {
  readonly id: string;
  readonly kind: AiParameterValueKind;
  readonly label: string;
  readonly labelRu: string;
...
export function getAiParameterDefinition(
  contract: AiNodeContract,
  parameterId: string,
): AiParameterDefinition | undefined {
  return contract.parameters.find((parameter) => parameter.id === parameterId);
}

export function getAiPortDefinition(
  ports: readonly AiPortDefinition[],
  portId: string,
): AiPortDefinition | undefined {
