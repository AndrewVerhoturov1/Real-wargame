...
import { getMapObjectCenter, getMapObjectHeightMetres, intersectSegmentWithMapObject } from '../map/MapObjectGeometry';
import {
...
export interface BallisticRayResult {
  ...
  hitUnitId?: string;
  hitZone?: HitZone;
  unitIntersection?: UnitHitIntersection;
...
interface TerrainTrace { hitDistanceMetres: number | null; minimumClearanceMetres: number | null; sampleCount: number; }
interface ObjectTrace { hitDistanceMetres: number | null; clearanceMetres: number | null; }
...
export function createBallisticTraceScratch(): BallisticTraceScratch {
  return { ignoredUnitIds: new Set<string>(), objectCandidates: [], objectQueryScratch: createMapObjectSpatialQueryScratch() };
}

export function createEmptyBallisticRayResult(): BallisticRayResult {
  return {
    shotId: '', hitType: 'none', travelledMetres: 0, flightTimeSeconds: 0,
    impactPoint: { xMetres: 0, yMetres: 0, zMetres: 0 },
    impactGridPosition: { x: 0, y: 0 }, clearanceMetres: null,
    objectCandidateCount: 0, unitCheckCount: 0, terrainSampleCount: 0,
  };
}

export function createBallisticTraceContext(map: TacticalMap, units: readonly UnitModel[]): BallisticTraceContext {
  return { map, units, objectSpatialIndex: getMapObjectSpatialIndex(map) };
}

export function traceBallisticRay(context: BallisticTraceContext, input: BallisticRayInput): BallisticRayResult {
  return traceBallisticRayPrepared(context, input, createBallisticTraceScratch(), createEmptyBallisticRayResult(), context.units);
}
...
export function traceBallisticRayPrepared(
  ...
  if (terrain.hitDistanceMetres !== null) nearest = chooseNearest(nearest, { type: 'terrain', distanceMetres: terrain.hitDistanceMetres });

  const segmentStart = { x: input.origin.xMetres / context.map.metersPerCell, y: input.origin.yMetres / context.map.metersPerCell };
  const endPoint = pointAlongRay(input.origin, direction, maximumDistanceMetres);
  const segmentEnd = { x: endPoint.xMetres / context.map.metersPerCell, y: endPoint.yMetres / context.map.metersPerCell };
  const objectCandidates = scratch.objectCandidates;
  context.objectSpatialIndex.querySegmentInto(segmentStart, segmentEnd, 0, objectCandidates, scratch.objectQueryScratch);
  for (const object of objectCandidates) {
    const trace = traceMapObject(context.map, input.origin, direction, maximumDistanceMetres, object, segmentStart, segmentEnd);
    minimumClearanceMetres = lowerNullable(minimumClearanceMetres, trace.clearanceMetres);
    if (trace.hitDistanceMetres !== null) nearest = chooseNearest(nearest, { type: 'object', distanceMetres: trace.hitDistanceMetres, objectId: object.id });
  }
  ...
  for (const unit of unitCandidates) {
    ...
    unitCheckCount += 1;
    const intersection = intersectRayWithUnitHitShapes(input.origin, direction, maximumDistanceMetres, unit, context.map);
    if (!intersection) continue;
    nearest = chooseNearest(nearest, {
      type: 'unit',
      distanceMetres: intersection.entryDistanceMetres,
      unitId: unit.id,
      zone: intersection.zone,
      unitIntersection: intersection,
    });
  }
  ...
  output.unitIntersection = nearest?.unitIntersection;
...
function traceTerrain(map: TacticalMap, origin: BallisticPoint3, direction: BallisticDirection3, maximumDistanceMetres: number): TerrainTrace {
  let minimumClearanceMetres: number | null = null;
  let sampleCount = 0;
  for (let distanceMetres = Math.min(TERRAIN_SAMPLE_STEP_METRES, maximumDistanceMetres); distanceMetres > 0 && distanceMetres <= maximumDistanceMetres + DISTANCE_EPSILON_METRES; distanceMetres += TERRAIN_SAMPLE_STEP_METRES) {
    ...
    if (!cell) return { hitDistanceMetres: sampleDistance, minimumClearanceMetres: 0, sampleCount };
    ...
    if (clearance <= TERRAIN_IMPACT_MARGIN_METRES) return { hitDistanceMetres: sampleDistance, minimumClearanceMetres: 0, sampleCount };
...
function traceMapObject(
  ...
  const vertical = intersectLinearSlab(origin.zMetres, direction.z * maximumDistanceMetres, bottomZ, topZ, footprint.entryT, footprint.exitT);
  if (vertical !== null) return { hitDistanceMetres: vertical * maximumDistanceMetres, clearanceMetres: 0 };
  ...
  return { hitDistanceMetres: null, clearanceMetres: minimumVerticalSeparation(zAtEntry, zAtExit, bottomZ, topZ) };
}

function intersectLinearSlab(origin: number, delta: number, minimum: number, maximum: number, lowerT: number, upperT: number): number | null {
  let enter = lowerT;
  let exit = upperT;
  if (Math.abs(delta) <= DISTANCE_EPSILON_METRES) return origin >= minimum && origin <= maximum ? enter : null;
...
function minimumVerticalSeparation(firstZ: number, secondZ: number, bottomZ: number, topZ: number): number {
...
function hitTypePriority(type: Exclude<BallisticHitType, 'none'>): number { return type === 'terrain' ? 0 : type === 'object' ? 1 : 2; }
function candidateIdentity(candidate: CandidateHit): string { return candidate.objectId ?? candidate.unitId ?? ''; }
function lowerNullable(current: number | null, next: number | null): number | null { if (next === null || !Number.isFinite(next)) return current; return current === null ? next : Math.min(current, next); }
function normalizeClearance(value: number | null): number | null { return value === null || !Number.isFinite(value) ? null : Math.max(0, value); }
