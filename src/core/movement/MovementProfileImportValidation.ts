...
export class MovementProfileImportError extends Error {
  ...
  constructor(issues: readonly MovementProfileImportIssue[]) {
    super(issues.map((item) => `${item.path}: ${item.messageEn}`).join('\n'));
...
const ENUM_FIELDS: ReadonlyArray<readonly [string, readonly string[]]> = [
  ...
  ['settings.noise.surfacePolicy', ['profile_multiplier', 'material_profile_adapter', 'material_profile_future']],
...
const NUMBER_FIELDS = [
  ...
  'settings.visibility.stealthSkillShare',
  ...
  'settings.weapon.aimPreparationMultiplier',
  'settings.weapon.weaponPreparationPenalty',
  'settings.restrictions.maximumWoundSeverity',
  'settings.restrictions.maximumSuppressionPercent',
  'settings.restrictions.minimumPhysicalCapability',
  'settings.restrictions.minimumSoldierSpeedMetersPerSecond',
  'settings.surface.materialSpeedMultiplier',
...
const TEXT_FIELDS = ['nameEn', 'nameRu', 'descriptionEn', 'descriptionRu'] as const;
...
export function validateMovementProfileImport(value: unknown): MovementProfileRegistryData {
  ...
  return normalizeMovementRegistryData(value);
}
...
function validateId(
  ...
  const migrated = resolveMovementProfileIdAlias(profile.id).id.trim().toLowerCase();
  if (isBuiltInMovementProfileId(migrated)) return migrated;
  try {
    return normalizeCustomMovementId(migrated);
  } catch (error) {
    issues.push(issue(
      `${path}.id`,
...
function validateProfileShape(
  ...
  if (preferredGait.present && !isSupportedImportedMovementGait(preferredGait.value)) {
    issues.push(issue(
      `${path}.preferredGait`,
      `Field must be one of: ${MOVEMENT_GAITS.join(', ')}. Legacy value "crouch" is accepted for migration.`,
      `Недопустимый способ движения. Допустимо: ${MOVEMENT_GAITS.join(', ')}. Legacy-значение «crouch» принимается для миграции.`,
    ));
  }
  ...
  if (template.present) {
    const normalizedTemplate = typeof template.value === 'string'
      ? resolveMovementProfileIdAlias(template.value).id.trim().toLowerCase()
      : '';
...
function normalizeReferenceId(value: string): string | null {
  const migrated = resolveMovementProfileIdAlias(value).id.trim().toLowerCase();
