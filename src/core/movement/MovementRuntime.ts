...
export interface MovementRuntimeState {
  ...
  /** Durable revision state; AI blackboard values are only a projection and may be recreated. */
  profileDefinitionRevision: number | null;
...
export function createMovementRuntime(
  ...
  return {
    ...
    profileDefinitionRevision: nullableInteger(record.profileDefinitionRevision),
    profileSelectionRevision: integer(record.profileSelectionRevision, 0, 0, Number.MAX_SAFE_INTEGER),
...
export function setMovementProfileRequest(
  ...
  const resolution = state.movementProfiles.resolveProfile(alias.id);
  setMovementRequest(unit, resolution.profile.id, source, gait ?? resolution.profile.preferredGait);
  if (source === 'unit_role') unit.unitRoleMovementProfileId = resolution.profile.id;
...
export function preparePhysicalMovementStep(
  ...
  const authorityProfileId = runtime.effectiveProfileId || runtime.requestedProfileId;
  const requestedResolution = state.movementProfiles.resolveProfile(authorityProfileId);
  let requestedProfile = requestedResolution.profile;
  const authoritySelected = authorityProfileId !== runtime.requestedProfileId
    || runtime.effectiveProfileSource === 'ai_override'
    || runtime.effectiveProfileSource === 'hard_safety';
  const authorityGait = authoritySelected ? requestedProfile.preferredGait : runtime.requestedGait;
  ...
  const execution = resolveExecution(
    ...
    requestedProfile,
    authorityGait,
    runtime.effectiveProfileSource,
  );
...
function resolveExecution(
  ...
  requestedGait: MovementGait,
  selectedSource: MovementProfileAuthoritySource,
): MovementExecution {
  const runtime = unit.movementRuntime;
