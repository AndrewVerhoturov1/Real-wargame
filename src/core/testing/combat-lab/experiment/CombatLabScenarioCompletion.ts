...
export function evaluateCombatLabCompletion(
  ...
  if (action.kind === 'stop_fire') return stopFireCompletion(conditionContext.state, observation);
...
function stopFireCompletion(state: SimulationState, observation: CombatLabCompletionObservationV1): CombatLabCompletionEvaluationV1 {
  const unit = findUnit(state, observation.actorUnitId);
  if (!unit) return failed('combat_lab_completion_unit_missing', 'Исполнитель остановки огня отсутствует.');
  return unit.infantryCombatRuntime.activeFireTask === null
    ? completed('combat_lab_stop_fire_completed', 'Огневая задача остановлена.')
    : pending('combat_lab_stop_fire_waiting', 'Ожидание остановки огневой задачи.');
}
...
function ammoActionCompletion(state: SimulationState, observation: CombatLabCompletionObservationV1, kind: 'reload' | 'transfer'): CombatLabCompletionEvaluationV1 {
  ...
  return terminal.status === 'completed'
    ? completed(`combat_lab_${kind}_completed`, terminal.resultRu)
    : failed(terminal.resultCode, terminal.resultRu);
}
...
function firstAidCompletion(state: SimulationState, observation: CombatLabCompletionObservationV1): CombatLabCompletionEvaluationV1 {
  ...
  return terminal.status === 'completed'
    ? completed('combat_lab_first_aid_completed', terminal.resultRu)
    : failed(terminal.resultCode, terminal.resultRu);
}
...
function actorForAction(experiment: CombatLabExperimentV1, state: SimulationState, action: CombatLabActionV1): UnitModel | null {
  if (action.kind === 'wait' || action.kind === 'transfer') return action.kind === 'transfer' ? resolveCombatLabRoleUnit(experiment, state, action.sourceRoleId) : null;
