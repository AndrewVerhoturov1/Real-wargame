...
Полный цикл для preview/r-init-потока:

```text
task branch -> работа -> перенос в real-wargame-preview -> проверка наличия изменений -> закрыть task branch -> пользователь тестирует preview -> только потом main
```
...
10. После успешного переноса результата в целевую preview-ветку (`real-wargame-preview`) временная task-ветка должна быть закрыта/удалена, чтобы не накапливались старые `codex/*` ветки.
