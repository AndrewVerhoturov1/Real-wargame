...
- выбирает режим внешней работы: полный R, короткий Q или W-class X;
...
- дорабатывает тот же PR по замечаниям.
...
Q используется только когда внешний web-chat имеет GitHub-доступ и может сам читать репозиторий, создать ветку и открыть PR.
...
- возвращать результат только текстом без PR, если задача предполагает изменения;
...
### X — W-class сильный внешний web/chat reasoning
...
X не использует ZIP. Результат X — ветка, PR и отчёт.

Authoritative truth order (порядок истины):
...
X-режим — v1. До проверки на практике Codex должен проверять X-PR так же тщательно, как Q-PR.

Подробно: `docs/ai/ZWORKER_MODES.md`.
...
9. Для Q- и X-режима внешний исполнитель должен вернуть Codex ссылку/номер PR, ветку, список изменённых файлов и статус проверок.
...
- `docs/ai/ZWORKER_MODES.md` — различие режимов R (полный zworker), Q (короткий GitHub-aware) и X (W-class web/chat reasoning).
...
3. Уточнить роль: внешний исполнитель, Codex-контролёр, OpenCode-руки или zworker-черновик.
4. Уточнить режим: R, Q или X.
5. Не читать весь репозиторий без необходимости.
6. Работать маленькими ограниченными шагами.
7. В отчёте честно писать, что проверялось и что не проверялось.
